...
function handleResultSet(err, res, callback) {
  ...
  var nullItemsCount = arr.filter((item) => !item).length

  if (nullItemsCount > 0) {
    debug(`BINGO! found ${nullItemsCount} null events. Filtering out now...`)
...
_.extend(Redis.prototype, {
  ...
  scan: function (key, cursor, handleKeys, callback) {
    ...
    var scanCount = 10; // redis default COUNT

    (function scanRecursive(curs) {
      self.client.scan(curs, 'MATCH', key, 'COUNT', scanCount, function (err, res) {
        if (err) {
          return callback(err);
        }
